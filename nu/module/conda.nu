...
export def-env activate [
    ...
    let new_env = if not $no_prompt {
        let old_prompt_command = if (has-env CONDA_OLD_PROMPT_COMMAND) {
            ...
            if (has-env 'PROMPT_COMMAND_RIGHT') {
        ...
        | insert PROMPT_COMMAND_RIGHT $new_prompt
    } else {
        $new_env
    }
...
export def-env deactivate [] {
    ...
    let-env PROMPT_COMMAND_RIGHT = $env.CONDA_OLD_PROMPT_COMMAND
    ...
    let-env PROMPT_COMMAND_RIGHT = if 'CONDA_OLD_PROMPT_COMMAND' in (env).name {
        $env.CONDA_OLD_PROMPT_COMMAND
    } else {
        $env.PROMPT_COMMAND_RIGHT
    }
